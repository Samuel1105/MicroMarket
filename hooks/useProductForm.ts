...
export const useProductForm = ({ producto, isEditMode = !!producto }: UseProductFormProps = {}) => {
    ...
    const methods = useForm<ProductoCompleteForm | ProductoCompleteEditForm>({
        resolver: zodResolver(isEditMode ? productoCompleteEditFormSchema : productoCompleteFormSchema),
    ...
    const processUpdateData = async (data: ProductoCompleteEditForm) => {
        try {
            ...
            const conversionesEliminadas = conversionesOriginales
                .filter(original => !conversionesActuales.find(actual => actual.id === original.id))
                .map(c => c.id);
            ...
            const apiData: ProductoUpdateApiData = {
                ...
                conversionesEliminadas: conversionesEliminadas.length > 0 ? conversionesEliminadas : undefined
            };
            console.log(apiData)
