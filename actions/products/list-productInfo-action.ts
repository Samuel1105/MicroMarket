...
export async function ListProduct() {
  try {
    ...
    const transformedProducts = products.map((product) => ({
      ...
      stockTotal: product.StockVenta.reduce(
        (total, stock) => total + Number(stock.cantidadDisponible),
        0,
      ),
      // Tomar el precio de venta más reciente o promedio
      precioVenta:
        product.StockVenta.length > 0
          ? Number(product.StockVenta[0].precioVenta)
          : undefined,
      // Encontrar la fecha de vencimiento más próxima
      fechaVencimiento:
        product.StockVenta.filter((stock) => stock.fechaVencimiento)
          .sort(
            (a, b) =>
              new Date(a.fechaVencimiento!).getTime() -
              new Date(b.fechaVencimiento!).getTime(),
          )[0]
          ?.fechaVencimiento?.toISOString() || null,
    }));
